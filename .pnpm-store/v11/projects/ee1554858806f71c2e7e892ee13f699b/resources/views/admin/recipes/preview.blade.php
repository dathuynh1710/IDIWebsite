...
<body>
    ...
    <main class="page">
        ...
        <div class="body">
            <section class="card"><h2>Nguyên liệu</h2>@forelse($recipe->ingredients as $item)<div class="ingredient"><span>{{ $item->getTranslation('name', $locale, false) ?: $item->getTranslation('name', 'vi', false) }}</span><strong>{{ trim(($item->quantity ?? '').' '.($item->getTranslation('unit', $locale, false) ?: '')) }}</strong></div>@empty<p>Chưa có nguyên liệu.</p>@endforelse</section>
            <section class="card"><h2>Cách làm</h2><div class="steps">@forelse($recipe->steps as $item)<div class="step">{{ $item->getTranslation('instruction', $locale, false) ?: $item->getTranslation('instruction', 'vi', false) }}</div>@empty<p>Chưa có hướng dẫn.</p>@endforelse</div></section>
        </div>
