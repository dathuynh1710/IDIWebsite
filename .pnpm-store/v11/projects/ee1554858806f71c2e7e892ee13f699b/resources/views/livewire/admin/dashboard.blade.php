    ...
    <div class="dashboard-grid">
        <x-ui.card title="Sản phẩm cập nhật gần đây">
            @if($recentProducts->isEmpty())
                <x-ui.empty-state title="Chưa có sản phẩm" description="Bắt đầu bằng việc tạo sản phẩm đầu tiên.">
                    @can('products.create')
                        <a class="button button-primary" href="{{ route('admin.products.create') }}" wire:navigate>Thêm sản phẩm</a>
                    @endcan
                </x-ui.empty-state>
            @else
        ...
        <x-ui.card title="Thao tác nhanh">
            <div class="quick-actions">
                @can('products.create')
                    <a href="{{ route('admin.products.create') }}" wire:navigate><x-ui.icon name="plus" /><span><strong>Thêm sản phẩm</strong><small>Tạo nội dung đa ngôn ngữ</small></span></a>
                @endcan
                @can('products.view')
                    <a href="{{ route('admin.products.index') }}" wire:navigate><x-ui.icon name="package" /><span><strong>Quản lý sản phẩm</strong><small>Lọc, sửa và xuất bản</small></span></a>
                @endcan
            </div>
        </x-ui.card>
    </div>
