<header class="admin-header">
    ...
    <div class="header-user"
        x-data="{ open: false, displayName: @js(auth()->user()->name), initial: @js(mb_strtoupper(mb_substr(auth()->user()->name, 0, 1))) }"
        @profile-updated.window="displayName = $event.detail.name; initial = $event.detail.initial"
        ...
        <button type="button" class="user-trigger" @click="open = !open" @click.outside="open = false" :aria-expanded="open.toString()">
            ...
            <span class="user-copy">
                <strong x-text="displayName"></strong>
                <small>Quản trị viên</small>
            </span>
            <x-ui.icon name="chevron-down" size="16" />
        </button>
        <div class="user-dropdown" x-show="open" x-transition x-cloak>
            <a href="{{ route('admin.profile') }}" wire:navigate @click="open = false"><x-ui.icon name="user" size="18" /> Hồ sơ</a>
