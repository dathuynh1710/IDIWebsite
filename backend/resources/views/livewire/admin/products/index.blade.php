<div>
    <x-admin.page-header title="Sản phẩm" description="Quản lý danh mục sản phẩm đa ngôn ngữ" :breadcrumbs="$breadcrumbs">
        <x-slot:actions>
            @can('products.create')
                <button class="button button-primary" type="button" wire:click="openCreateModal"><x-ui.icon name="plus" size="18" /> Thêm sản phẩm</button>
            @endcan
        </x-slot:actions>
    </x-admin.page-header>
    ...
    @if($showFormModal)
        <div class="admin-form-modal" x-data x-on:keydown.escape.window="$wire.closeFormModal()" role="dialog" aria-modal="true" aria-label="Biểu mẫu sản phẩm">
