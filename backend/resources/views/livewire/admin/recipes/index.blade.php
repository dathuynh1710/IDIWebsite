    ...
    <section class="card category-list-card">
        ...
        @else
            ...
            <div class="recipe-mobile-list">
                @foreach($recipes as $item)
                    <article class="product-mobile-card" wire:key="mobile-recipe-{{ $item->id }}"><div class="product-cell"><div class="product-thumb">@if($item->featuredMedia)<img src="{{ $item->featuredMedia->url }}" alt="">@else<x-ui.icon name="image" />@endif</div><div><strong>{{ $item->getTranslation('title', $locale, false) ?: 'Chưa có bản dịch' }}</strong><small>{{ ($item->preparation_time ?? 0) + ($item->cooking_time ?? 0) }} phút</small></div></div><div class="mobile-card-actions">@can('recipes.update')<a class="button button-secondary" href="{{ route('admin.recipes.edit', $item) }}" wire:navigate><x-ui.icon name="edit" size="18" /> Sửa</a>@endcan</div></article>
                @endforeach
            </div>
