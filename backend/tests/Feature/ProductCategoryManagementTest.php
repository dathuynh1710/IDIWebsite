...
class ProductCategoryManagementTest extends TestCase
{
    ...
    public function test_product_editor_can_view_create_and_edit_category_pages(): void
    {
        ...
        $this->actingAs($user)->get('/admin/product-categories')->assertOk()->assertSee('Danh mục sản phẩm')->assertSee('Cá Fillet');
    ...
    public function test_category_create_modal_opens_without_navigation(): void
    {
        Livewire::actingAs($this->productEditor())
            ->test(ProductCategoryIndex::class)
            ->call('openCreateModal')
            ->assertSet('showFormModal', true)
            ->assertSee('Biểu mẫu danh mục');
    }
