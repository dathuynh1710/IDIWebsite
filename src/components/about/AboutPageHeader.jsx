...
export default function AboutPageHeader({ eyebrow = 'Về IDI', title, description }) {
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-ocean-deep pt-28 text-white sm:pt-32">
        ...
        <div className="container relative pb-14 pt-10 sm:pb-16 sm:pt-14">
          <span className="text-xs font-extrabold uppercase tracking-[0.18em] text-coral-light">{eyebrow}</span>
        ...
        <div className="border-t border-white/10 bg-white/5">
          <nav aria-label="Điều hướng Về IDI" className="container flex gap-2 overflow-x-auto py-3">
            {ABOUT_NAV.map((item) => (
              <NavLink
                key={item.href}
                to={item.href}
